...
const MapComponent = () => {
    ...
    d3.json('https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.json')
      .then((worldData) => {
        ...
        const featureCollection: GeoJSON.FeatureCollection<GeoJSON.Geometry> = {
          ...
          features: Array.isArray(countries.features) ? countries.features : [countries],
        };
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', minWidth: "1200px", backgroundColor: '#ffffff' }}>
      <div style={{ flex: 1, width: '100%' }}>
        <svg ref={svgRef} style={{ width: '100%', height: '100%' }}></svg>
      </div>
      <div style={{ width: '500px', padding: '20px', backgroundColor: '#f9f9f9', height: '100vh', overflowY: 'auto' }}>
        <h2 style={{ fontSize: '24px', fontWeight: 'bold', color: '#333' }}>Country Info</h2>
        <p style={{ fontSize: '16px', color: '#555' }}>{countryInfo}</p>
        <h3 style={{ fontSize: '20px', color: '#333' }}>Zoom</h3>
        <input
          type="range"
          min="100"
          max="500"
        ...
        />
        <p style={{ fontSize: '16px', color: '#333' }}>Zoom Level: {zoomLevel}</p>
      </div>
    </div>
  );
};
